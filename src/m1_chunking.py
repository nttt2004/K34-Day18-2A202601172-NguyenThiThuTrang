...
def chunk_hierarchical(text: str, parent_size: int = HIERARCHICAL_PARENT_SIZE,
    ...
    def _group(units: list[str], max_size: int) -> list[str]:
        groups = []
        current = ""
        for unit in units:
            if current and len(current) + len(unit) + 2 > max_size:
    ...
    parent_texts = _group(paragraphs, parent_size)
    ...
    for parent_text in parent_texts:
        ...
        child_texts = _group(sentences, child_size)
